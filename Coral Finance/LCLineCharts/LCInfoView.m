...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {		
        ...
        if(!self.textColor) self.infoLabel.textColor = [UIColor whiteColor];
        else self.infoLabel.textColor = self.textColor;
...
- (void)recalculateFrame {
    ...
    {
        theFrame.origin.y = self.tapPoint.y - theFrame.size.height + 2 * SHADOWSIZE + 11;
